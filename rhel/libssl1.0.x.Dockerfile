...
RUN wget -c https://www.openssl.org/source/openssl-1.0.2u.tar.gz
RUN tar -xzvf openssl-1.0.2u.tar.gz
...
RUN cd /etc/ld.so.conf.d/ && echo "/usr/local/ssl/lib64" > openssl-1.0.2u.conf
RUN ldconfig -v
